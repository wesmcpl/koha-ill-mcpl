package Koha::Illbackends::Mcpl::Base;
...
Koha ILL implementation for the Marshall County Public Library.
...
=head2 On the MCPL backend

The MCPL backend is FreeForm backend modified for Marshall County Public Library.
It provides the end user with some mandatory fields in a form as
...
We only want to display a bit of text to let staff
...
In the backend this simply means displaying text to the librarian
...
Given supplied metadata from a mcpl request, create a basic biblio
...
sub _mcpl2biblio {
    ...
    return 0 unless ($metadata->{type} eq 'book' || $metadata->{type} eq 'abook' || $metadata->{type} eq 'dvd');
...
Wes Smith <wsmith@marshallcolibrary.org>
